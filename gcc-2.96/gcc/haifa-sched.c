...
/* Camelot matching: non-zero when the post-reload dependence analysis must
   assume MEM1 and MEM2 conflict because -fno-sched-alias took the alias oracle
   out of it.  The reference objects behave that way at the load/store pairs
   this fork reorders: a store and a later load of two different constant
   offsets off one base are provably independent, so the fork lets the load's
   longer dependence chain pull it in front of the store, while the reference
   keeps the two in the order they were written.  Constant-pool references are
   exempt -- they are RTX_UNCHANGING_P and nothing writes them -- and so is
   read-after-read, which stays governed by read_dependence.  */

static int
sched_assume_conflict_p (mem1, mem2)
     rtx mem1, mem2;
{
  return (! flag_schedule_alias
	  && ! RTX_UNCHANGING_P (mem1)
	  && ! RTX_UNCHANGING_P (mem2));
}
     ...
     rtx insn;
{
  ...
  if (GET_CODE (dest) == REG)
      ...
      else
	{
	  ...
	  while (pending)
	    {
	      if (sched_assume_conflict_p (XEXP (pending_mem, 0), dest)
		  || anti_dependence (XEXP (pending_mem, 0), dest))
	  ...
	  while (pending)
	    {
	      if (sched_assume_conflict_p (XEXP (pending_mem, 0), dest)
		  || output_dependence (XEXP (pending_mem, 0), dest))
	...
	while (pending)
	  {
	    ...
	    /* Camelot matching: a conflict the oracle did not find but
	       -fno-sched-alias forces is recorded as an ordering edge, not a
	       true dependence: the point is only that the load must not move
	       above the store, and a zero-cost edge says that without also
	       lengthening the store's path to the end of the block, which
	       would change its priority against unrelated insns.  */
	    else if (sched_assume_conflict_p (XEXP (pending_mem, 0), x))
	      add_dependence (insn, XEXP (pending, 0), REG_DEP_ANTI);
...
/* Camelot matching: the destination hard register of INSN's single SET when
   INSN is one of the argument setters in front of a call -- a call is among
   its forward dependents -- and the target names that register in
   SCHED_DEST_ORDER_REGNO_P.  -1 otherwise.  rank_for_schedule uses it under
   -fsched-low-dest-first to break a tie in ascending register order.  The
...
/* Camelot matching: non-zero if INSN's single SET stores to memory.
   rank_for_schedule uses it under -fsched-store-first.  */

static int
sched_store_insn_p (insn)
     rtx insn;
{
...
/* Camelot matching: the priority rank_for_schedule compares.  Under
   ...
   ordered by the rules further down -- which, with equal dependent counts, is
   original order.  Without the flag this is just INSN_PRIORITY.  */

static int
sched_effective_priority (insn)
     rtx insn;
{
  if (flag_schedule_store_first && sched_store_insn_p (insn))
    return INT_MAX;

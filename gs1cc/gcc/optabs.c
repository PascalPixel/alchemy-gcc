     ...
     enum optab_methods methods;
{
  ...
  if (methods != OPTAB_MUST_WIDEN
	...
	temp = gen_reg_rtx (mode);
      ...
      pat = ((insn_gen_fn3) GEN_FCN (icode)) (temp, xop0, xop1);
      if (pat)
	{
     ...
     int unsignedp;
{
  ...
  if (binoptab->handlers[(int) mode].insn_code != CODE_FOR_nothing)
	...
	abort ();
      ...
      pat = ((insn_gen_fn4) GEN_FCN (icode)) (targ0, xop0, xop1, targ1);
      if (pat)
	{
	  emit_insn (pat);
	  return 1;
     ...
     int unsignedp;
{
  ...
  if (unoptab->handlers[(int) mode].insn_code != CODE_FOR_nothing)
	...
	temp = gen_reg_rtx (mode);
      ...
      pat = ((insn_gen_fn2) GEN_FCN (icode)) (temp, xop0);
      if (pat)
	{
	  if (GET_CODE (pat) == SEQUENCE
     ...
     enum rtx_code code;
{
  ...
  pat = ((insn_gen_fn2) GEN_FCN (icode)) (temp, op0);
     ...
     rtx label;
{
  ...
  do
	...
	{	  
	  ...
	  if (icode != CODE_FOR_nothing
	      && (*insn_data[icode].operand[0].predicate) (test, wider_mode))
	    {
	      ...
	      emit_jump_insn (((insn_gen_fn4) GEN_FCN (icode)) (test, x, y, label));
	      return;
	    }
	}
      ...
      if (y == CONST0_RTX (mode) && icode != CODE_FOR_nothing)
	{
	  ...
	  emit_insn (((insn_gen_fn1) GEN_FCN (icode)) (x));
	  if (label)
      ...
      if (icode != CODE_FOR_nothing)
	{
	  ...
	  emit_insn (((insn_gen_fn2) GEN_FCN (icode)) (x, y));
	  if (label)
	    emit_jump_insn ((*bcc_gen_fctn[(int) comparison]) (label));
	  return;
	}
     ...
     int unsignedp;
{
  ...
  insn = ((insn_gen_fn4) GEN_FCN (icode)) (subtarget, comparison, op2, op3);
     ...
     rtx x, y;
{
  ...
  return (((insn_gen_fn3) GEN_FCN (icode)) (x, x, y));
}
     ...
     rtx x, y;
{
  ...
  if (GET_MODE_CLASS (mode) == MODE_CC && insn_code == CODE_FOR_nothing)
      ...
      else
	{
      ...
      return (((insn_gen_fn2) GEN_FCN (insn_code)) (x, y));
    }
...
gen_extend_insn (x, y, mto, mfrom, unsignedp)
     ...
     int unsignedp;
{
  return (((insn_gen_fn2) GEN_FCN (extendtab[(int) mto][(int) mfrom][unsignedp != 0])) (x, y));
}

  ...
  rtx op1, op2 ATTRIBUTE_UNUSED, tcode ATTRIBUTE_UNUSED;
{
  ...
  if (HAVE_conditional_trap
      && cmp_optab->handlers[(int) mode].insn_code != CODE_FOR_nothing)
    {
      rtx insn;
      start_sequence();
      emit_insn (((insn_gen_fn2) GEN_FCN (cmp_optab->handlers[(int) mode].insn_code)) (op1, op2));
